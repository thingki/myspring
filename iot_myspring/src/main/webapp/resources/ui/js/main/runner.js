!function(){
	...
	function c(n){
		if(null===n||"undefined"==typeof n)return 1;var t,o=e(n);
		...
		if("[object Object]"===o){
			...
			for(t=0;t<r.length;t++){
				var c=n[r[t]];
				if(u={}.toString.call(c),"[object Function]"===u||"[object Object]"===u||"[object Array]"===u)
					return 2
					}
			return 1
			}
	...
	function i(n,t,o){
		var c,u,a="",l=[];if(o=o||"",t=t||[],null===n)return"null";
		...
		for(u=0;u<t.length;u++)if(n===t[u])return"[circular "+a.slice(1)+("outerHTML"in n?" :\n"+n.outerHTML.split("\n").join("\n"+o):"");
		if(t.push(n),"[object Array]"==a){
			for(c=0;c<n.length;c++)l.push(i(n[c],t));
			return"["+l.join(", ")+"]"}if(a.match(/Array/))return a;
			var f=a+" ",s=o+"  ";
			if(o.length/2<2){
				var b=[];try{
					for(c in n)b.push(c)
	...
	function u(){if(window.console)for(var n=0;n<f.length;n++)!function(){
		var t=f[n];window.console[t]&&(window.console[t]=function(){
			for(var n=[].slice.call(arguments),e=[],r=[],u=0;u<n.length;u++)o(n[u])?(r.push(i(n[u].outerHTML)),e.push(1)):(r.push(i(n[u])),e.push(c(n[u])));
			s.postMessage(["console",{"function":t,arguments:r,complexity:Math.max.apply(null,e)}],"*"),this.apply(console,n)}.bind(console[t]))}()}function a(n){return!!n.origin.match(/codepen/)&&("object"==typeof n.data&&"command"===n.data.type)}function l(n){if(a(n)){var t=n.data.command;
			try{
				var e=window.eval(t)
				}catch(n){
			...
			}var f=["log","error","warn","info","debug","table","time","timeEnd","count","clear"],s=window.parent;n()}();
